...
import { useClickedOutside, useHoveredOutside } from '../Hooks'; // set element's 'elem' position at 'coords'

function positionElementAt(elem, coords) {
  elem.style.position = "absolute";
  ["left", "right", "top", "bottom"].forEach(coord => {
    if (coords[coord]) elem.style[coord] = coords[coord];else elem.style[coord] = "";
  });
}
...
export default function Menu(props) {
  const {
    ...
    menuTerminalItemAttributes,
    menuNonterminalItemAttributes,
    activateOn,
    onShow,
    onClose,
    showAt,
    ...
    closeOn,
    closeTimeout,
    closeImmediatelyOnHoverOnAnotherMenuItem,
  ...
  useClickedOutside(container, closeOn === "click" || closeOn === 'hover' && closeImmediatelyOnClickOnAnotherMenuItem ? deactivate : () => {}); // we should not use hoveredOutside if menu is not active

  useHoveredOutside(container, active && closeOn === "hover" || closeOn === 'click' && closeImmediatelyOnHoverOnAnotherMenuItem ? deactivate : () => {}, closeTimeout, closeImmediatelyOnHoverOnAnotherMenuItem ? [activatorAttributes.className, containerAttributes.className] : []);
  ...
  function positionMenuAt(event) {
    ...
    switch (showAt) {
      ...
      case "custom":
        positionElementAt(menu.current, { ...customShowPos
        });
        break;
  ...
  function toggle(event) {
    if (active) deactivate();else activate(event);
  }
  ...
  function buildMenu() {
    return React.createElement("div", _extends({
      ...
      ref: menu
    }, menuContainerAttributes), menuItems.map((item, index) => {
      const isTerminal = item.type !== Menu;
      return isTerminal ? React.createElement("div", _extends({
        key: index
      }, menuTerminalItemAttributes), item) : React.createElement("div", _extends({
        key: index
      }, menuNonterminalItemAttributes), item);
    }));
  }
...
Menu.propTypes = {
  ...
  menuNonterminalItemAttributes: PropTypes.object,
  // another menu
