...
export default function useHoveredOutside(ref, handler, timeout = 0, exceptionsClassesCloseImmediately = []) {
  ...
  function handleUnhover(event) {
    const isException = exceptionsClassesCloseImmediately.some(exceptionClass => event.relatedTarget && event.relatedTarget.closest(`.${exceptionClass}`));
    if (isException) handler();else setHovered(false);
  }
  ...
  useEffect(() => {
    const current = ref.current; // Bind the event listener
