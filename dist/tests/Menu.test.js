...
describe("<Menu />", () => {
  it("renders simple menu correctly", () => {
    ...
    expect(document.querySelectorAll(".ababa").length).toEqual(0); // rendering without unnecessary options
  ...
  it("renders multilevel menu correctly", () => {
    ...
    expect(document.querySelectorAll(".ababa").length).toEqual(0); // finding all items

    const menuItems = [...document.querySelectorAll(".menu-item")];
    [1, 2, 3, 4, 5, 6].forEach(number => {
      expect(menuItems.find(menuItem => menuItem.innerHTML.includes("Item" + number))).toBeTruthy();
  ...
  it("check activateOn", () => {
    // click
    const choicer = mount(React.createElement(Choicer, null));
    ...
    choicer.find(".activator").at(0).simulate("click");
    expect(choicer.find(".submenu").at(0).getDOMNode().style.display).toEqual("none"); // hover(can be checked only with enzyme)
  ...
  it("check onShow(), onClose()", () => {
    let a = 0;
    act(() => {
      render(createMenu({ ...menuDefaultOpts,
        activateOn: "click",
        onShow: () => {
          a = 1;
        },
  ...
  it("check showAt, customShowPos", () => {
    ...
    expect(submenuStyle.right).toEqual("100%"); // right
    ...
    expect(submenuStyle.top).toEqual("0px");
    expect(submenuStyle.left).toEqual("100%"); // custom
    ...
    expect(submenuStyle.left).toEqual("50%"); // don't checking natural, because it just adds menu in dom, also don't checking cursor because we are not rendering
  });
  it("closeOn none", () => {
    ...
    expect(choicer.find(".submenu").at(1).getDOMNode().style.display).toEqual("");
    choicer.find(".activator").at(1).simulate("click");
    expect(choicer.find(".submenu").at(1).getDOMNode().style.display).toEqual("none"); // not closing if onClose == 'none' and activateOn == 'hover' - because we are still hovering, it is expected behaviour
  ...
  it("closeOn click", () => {
    const map = {}; // doing map of array, as one element can have more than 1 event listeners
    ...
    choicer.find(".activator").at(1).simulate("click");
    expect(choicer.find(".submenu").at(1).getDOMNode().style.display).toEqual("");
    choicer.find(".activator").at(2).simulate("click"); // need to wrap in act because we are simulating pure DOM action

    act(() => {
      map["click"].forEach(clickEL => {
        clickEL({
          bubbles: true,
          target: choicer.find(".activator").at(2).getDOMNode()
        });
      });
    });
    expect(choicer.find(".submenu").at(2).getDOMNode().style.display).toEqual(""); // 1 menu is hidden now
  ...
  it("closeOn hover, close timeout", done => {
    const map = {};
    const choicer = mount(React.createElement(Choicer, null)); // doing map of array, as one element can have more than 1 event listeners
    ...
    expect(choicer.find(".submenu").at(2).getDOMNode().style.display).toEqual("none"); // to show element
  ...
  it("ciohoami", done => {
    ...
    act(() => {
      cont0map['mouseleave'][0]({
        relatedTarget: choicer.find(".activator").at(1).getDOMNode()
      });
  ...
  it("ciocoami", done => {
    ...
    act(() => {
      map['click'][0]({
        target: choicer.find(".activator").at(1).getDOMNode()
      });
    });
    expect(choicer.find(".submenu").at(1).getDOMNode().style.display).toEqual("");
    setTimeout(() => {
